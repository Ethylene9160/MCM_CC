...
def data_processing(n_train,historical,batch_size,total,header_list, base_path = '../'):
    data = {}
    features = {}  # 元素为list
    labels = {}
    for i in range(1, total):
        data_path = base_path+f'statics/29splits/session{i}.csv'
    ...
    # plt.figure()
    # plt.plot(test_labels[:,0,1].reshape(-1,1))
    # plt.title('test label in lstm_method')
    # plt.show()
